...
def create():
    ...
    # if correct, it will return the object instance
    ...
    if not user:
        ...
        if not result:
            ...
            # takes the id of the user object and saves it to the login_manager, it will then pass it to the 'load_user' function
            login_user(user)
            access_token = create_access_token(identity=user.id)
            flash('Succesfully Logged In')
            flash(f'JWT_TOKEN = {access_token}')
            return redirect(url_for('home'))


@sessions_blueprint.route("/google_login")
def google_login():
    # Oauth 3) This generates the URL that the user should be redirected to after authorization is complete
    redirect_uri = url_for('sessions.authorize', _external=True)
    # Oauth 4) This authorizes a particular URL to have access to the authentication code and opens up the google sign in
    return oauth.google.authorize_redirect(redirect_uri)


@sessions_blueprint.route("/authorize/google")
def authorize():
    # Oauth 5) This exchanges the auth code for a token
    oauth.google.authorize_access_token()
    # Oauth 5) Using the token to make an API call to retrieve data (end)
...
# Need to have different route names if there are multiple functions
@sessions_blueprint.route('/setting_privacy', methods=['POST'])
@login_required
def update_is_public():
    # HTML values are always given as a string so we need to convert them ourselves
    is_public_to_update = request.form.get('is_public')
